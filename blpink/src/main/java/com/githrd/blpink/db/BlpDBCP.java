...
public class BlpDBCP {
	...
	/* 
	 	이 클래스를 누군가 new 시키면
	  	context.xml 파일에 등록된 자원을 가지고 오도록 해야 한다.
	  	이처럼 context.xml 파일에 등록된 자원을 가지고 온느 기법을
	...
	*/
	...
	public BlpDBCP() {
		try {
			...
			/*
				...
				==> 서버의 환경중에서 jdbc/TestDB라는 이름의 자원을 찾아주세요
					==> 커넥션 풀을 찾아주세요.
					
				==> 이 작업이 무사히 완료되면
					커넥션 풀을 찾아서 기억해뒀다는(ds 변수에)는 것이 된다.
			 */
		} catch(Exception e) {
			e.printStackTrace();
		}
	...
	// PreparedStatement가 필요하면 만들어주는 함수
