...
/**
 *	이 클래스는 .blp 라는 확장자로 요청이 된 경우
 *	요청 내용을 분석하 실행하는 클래스를 선택해서 실행(디스패치)시켜줄 서블릿클래스이다.
 * @author 이용현
 * @since 2022/05/11
 * @version v.1.0
 * @see
...
//			System.out.println("#### dispatch path : " + path); ==> /C:/class/jsp/source/.metadata/.plugins/org.eclipse.wst.server.core/tmp0/wtpwebapps/whistle/WEB-INF/classes/com/githrd/whistle/dispatch/../resources/blp.properties
		...
		// 위 properties에 기억된 클래스이 경로를 이용해서 Map을 완성한다.
		...
		while(en.hasMoreElements()) {
			...
			// 실행할 클래스 경로 꺼내고
//			System.out.println(prop);	==> {/main.blp=com.githrd.whistle.controller.MainForm}
			String classPath = prop.getProperty(key);
//			System.out.println(classPath);	==> com.githrd.whistle.controller.MainForm
			try {
				Class tmp = Class.forName(classPath);
//				System.out.println(tmp);	==> class com.githrd.whistle.controller.MainForm
				// 실제 실행 가능한 객체로 만들고
	...
	/*
	 	...
	 	실행할 객체(클래스)를 꺼내서 요청을 처리해 주면 된다.
	 */
	/*
	 	참고 ]
	 		이 함수를 실행할 때 이 함수가 뷰를 처리하는 방식이
	 		forward, redirect, ajax도 동시에 처리를 해야한다.
	 		
	 		따라서 각각의 작업을 구분해 줄 수 있는 변수를 만들어서 처리하기로 하자.
...
//		System.out.println("# service full : " + full); --> # service full : /whistle/main.blp
//		System.out.println("# service domain : " + domain); --> # service domain : /whistle
		// 3. 실제요청을 알아내고
